    ...
    Papa.parse(`2024raw.csv`, {
        ...
        complete: function (results) {
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                team_batting.push("Unknown");
                team_fielding.push("Unknown");
                date.push(data[i][1]);
            }

            for(let i = batter.length - 1; i >= 0; i --) {
                output += `${batter[i]}, ${batter_name[i]}, ${description[i]}, ${des[i]}, ${game_pk[i]}, ${hc_x[i]}, ${hc_y[i]}, ${hit_angle[i]}, ${hit_distance[i]}, ${hit_speed[i]}, ${inning[i]}, ${outs[i]}, ${pitcher[i]}, ${pitcher_name[i]}, ${play_id[i]}, ${team_batting[i]}, ${team_fielding[i]}, ${date[i]}\n`;
            }
            console.log(output);
