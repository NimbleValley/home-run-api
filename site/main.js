  ...
  Papa.parse(url, {
    ...
    complete: function (results) {
      ...
      for (var i = 1; i < data.data.length; i++) {
        if (currentYear) {
          if ((outcomeValue == "hit" && data.data[i][2] == "field_out") || (outcomeValue == "field_out" && data.data[i][2] != "field_out")) {
            continue;
          }
        } else {
        ...
        if (currentYear) {
          if (data.data[i][15] != document.getElementById("stadium-select-search").value && document.getElementById("stadium-select-search").value != "void") {
            continue;
          }
        } else {
        ...
        if (currentYear) {
          temp.innerText = data.data[i][17] + " | " + hitResult + " | " + data.data[i][3] + " | " + results.data[i][16] + " vs " + results.data[i][15] + " | " + results.data[i][8] + "ft";
        } else {
        ...
        // Filter
        if (currentYear) {
          if (data.data[i][0] == playerId || playerId == -1) {
            dataCount++;
...
function selectIndex(results, index) {
  ...
  if (currentYear) {
    abb = results.data[index][15];
    console.warn(abb)
    swapStadium(false);
    initialHeight = 3.15 * DISTANCE_SCALE * 3;
    hitDistance = results.data[index][8] * DISTANCE_SCALE;
    hc_x = results.data[index][5];
    hc_y = results.data[index][6];
    launch_speed_fts = results.data[index][9] * 5280 / 3600;
    launch_angle = results.data[index][7] * Math.PI / 180;
